...
from MobileAgent.icon_localization import det, upload2S3
...
from utils.page_judge import judge_same_page, judge_element, find_a_baseline_page
from utils.mysql import get_all_pages_from_database
import pandas as pd
...
# Your instruction
instruction = "左右滑动手机页面，找到美团众包app，打开美团众包，打开侧边栏查看骑手姓名，跳过所有弹窗。"
appType = 1 # 1 represent zhongbao


model = 'gpt-4o-2024-08-06'
# Your GPT-4o API URL
API_url = "https://oneapi.sankuai.com/v1/chat/completions"

# Your GPT-4o API Token
token = "sk-WzlhuMbfFC2xH7mqA4673226E9E241E78"
...
qwen_api = "sk-8ebeb2a3c9df411f9fecbcb96df6d7a3"
...
def get_perception_infos(adb_path, screenshot_file):
    ...
    # get icon and crop icon-image
    # coordinates = det(screenshot_file, "icon", groundingdino_model)
    coordinates = []
    coordinates.append([0, 0, 100, 100])
...
if __name__ == "__main1__":
    ...
    all_pages_from_db = get_all_pages_from_database(appType=appType)
    screenshot_s3location = upload2S3("/Users/chenjiahua05/pythonProject/MobileAgent/Mobile-Agent/Mobile-Agent-qwen/screenshot/test_screenshot.jpg", "screenshot")
    baseline_page_result = find_a_baseline_page(3, screenshot_s3location, model, API_url, token)
    page_id = baseline_page_result.split("### id ###")[-1].replace("\n", " ").replace("  ", " ").strip()
    print(page_id)
...
if __name__ == "__main__":
    ...
    iter = 0
    df = pd.DataFrame(data=None, columns=['pages_id', 'element_type', 'element_content', 'clickable', 'next_page_id', 'desc', 'actual_page'])
    while True:
        iter += 1

        print("-" * 50 + str(iter) + "-" * 50)
        # first iterator :  get perception info of image and make sure that if there has keyboard
        if iter == 1:
            screenshot_file = "./screenshot/screenshot.jpg"
            perception_infos, width, height = get_perception_infos(adb_path, screenshot_file)  # [{'text': 'text: 美团', 'coordinates': [496, 44]}], width = 935, height = 1983
            shutil.rmtree(temp_file) # 递归删除文件夹
            os.mkdir(temp_file)      # 创建文件夹
        ...
        # add assistant output_action
        output_action = inference_chat(chat_action, 'gpt-4o-2024-08-06', API_url, token) # output_action: thought, action, operator
        thought = output_action.split("### Thought ###")[-1].split("### Action ###")[0].replace("\n", " ").replace(":", "").replace("  ", " ").strip()
        action = output_action.split("### Action ###")[-1].split("### Operation ###")[0].replace("\n", " ").replace("  ", " ").strip()
        summary = output_action.split("### Operation ###")[-1].replace("\n", " ").replace("  ", " ").strip()
        chat_action = add_response("assistant", output_action, chat_action)

        # output the decision of the ChatGPT
        ...
        if memory_switch:
            ...
            output_memory = inference_chat(chat_action, 'gpt-4o-2024-08-06', API_url, token)
        ...
        # save the screenshot as the old
        ...
        if reflection_switch:
            ...
            output_reflect = inference_chat(chat_reflect, 'gpt-4o-2024-08-06', API_url, token)
            ...
            if 'A' in reflect:
                ...
                output_planning = inference_chat(chat_planning, 'gpt-4-turbo-2024-04-09', API_url, token)
        ...
        # save element
        all_pages_from_db = get_all_pages_from_database(appType=appType)
        screenshot_s3location = upload2S3(
            "/Users/chenjiahua05/pythonProject/MobileAgent/Mobile-Agent/Mobile-Agent-qwen/screenshot/screenshot.jpg",
            "screenshot")
        baseline_page_result = find_a_baseline_page(appType, screenshot_s3location, model, API_url, token)
        page_id = baseline_page_result.split("### id ###")[-1].replace("\n", " ").replace("  ", " ").strip()
        print("The same page id is :", page_id)
        window_dump = ET.parse('/Users/chenjiahua05/pythonProject/MobileAgent/Mobile-Agent/Mobile-Agent-qwen/screenshot/window_dump.xml')

        ## 判断是否是同一个页面，找到执行页面的页面id
        # screenshot_comp_result = judge_same_page(last_screenshot_file, screenshot_file, 'gpt-4-turbo-2024-04-09', API_url, token)
        # # print(screenshot_comp_result)
        # ret = screenshot_comp_result.split("### Conclusion ###", 1)[-1].replace("\n", " ").replace("  ", " ").strip()
        # print("The comp of two screenshot is : \n " + ret)


        ## 是新页面 and 判断表中没有存储过
        for i in range(len(last_perception_infos)) :
            last_perception_info = last_perception_infos[i]
            # last_perception_info = {'text': 'text: N*9', 'coordinates': [662, 76]}
            # last_perception_info.keys()
            print("last_perception_info : ", last_perception_info)
            element_type = last_perception_info["text"].split(":")[0]
            content = last_perception_info["text"].split(":")[1]

            judge_ret = judge_element(page_id, last_perception_info, window_dump, appType, model, API_url, token)

            print("judge_ret : \n", judge_ret)
            clickable = judge_ret.split("### Clickable ###")[-1].split("### id ###")[0].replace("\n", " ").replace("  ", " ").strip()
            print("clickable : \n", clickable)

            next_id = -1
            if clickable:
                next_id = judge_ret.split("### id ###", 1)[-1].replace("\n", " ").replace("  ", " ").strip()
                print(next_id)

            if i == 0:
                new_row = pd.Series([page_id, element_type,  content, clickable, next_id, "", screenshot_s3location], index=df.columns)
            else:
                new_row = pd.Series([page_id, element_type, content, clickable, next_id, "", ""], index=df.columns)
            # 使用append()方法添加新行
            df = pd.concat([df, pd.DataFrame(new_row.to_frame().T)], ignore_index=True)
            # print(df)


    current_time = strftime('%Y-%m-%d %H-%M-%S', localtime())
    df.to_excel("./screenshot/banma_pages_element_{}.xlsx".format(current_time))
    print("save element xlsx done!")
